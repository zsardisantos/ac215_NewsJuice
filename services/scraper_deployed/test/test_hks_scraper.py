...
from hks_scraper import HksArticleScraper  # noqa: E402
...
def article_soup():
    return _load_html("HKS_sample_article.html")


@pytest.fixture(scope="module")
def topic_soup():
    return _load_html("HKS_sample_topic_page.html")


@pytest.fixture
def scraper():
    with patch("hks_scraper.PostgresDBManager"):
        s = HksArticleScraper(headless=True, test_mode=True)
        s.db_manager = MagicMock()
        s.db_manager.filter_new_urls.return_value = []
        yield s


class TestHksScraper:
    ...
    def test_extract_article_author(self, scraper, article_soup):
        ...
        assert author is None  # HKS articles don't have clear authors
    ...
    @patch("hks_scraper.dateparser.parse", return_value=None)
    ...
    # Scrape flow
    @patch("hks_scraper.sync_playwright")
