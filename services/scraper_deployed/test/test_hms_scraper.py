...
from hms_scraper import HmsArticleScraper  # noqa: E402
...
def article_soup():
    return _load_html("HMS_sample_article.html")


@pytest.fixture(scope="module")
def topic_soup():
    return _load_html("HMS_sample_topic_page.html")


@pytest.fixture
def scraper():
    with patch("hms_scraper.PostgresDBManager"):
        s = HmsArticleScraper(headless=True, test_mode=True)
        s.db_manager = MagicMock()
        s.db_manager.filter_new_urls.return_value = []
        yield s


class TestHmsScraper:
    ...
    @patch("hms_scraper.dateparser.parse", return_value=None)
    ...
    # Scrape flow
    @patch("hms_scraper.sync_playwright")
