...
from seas_scraper import SeasArticleScraper  # noqa: E402
...
def article_soup():
    return _load_html("SEAS_sample_article.html")


@pytest.fixture(scope="module")
def topic_soup():
    return _load_html("SEAS_sample_topic_page.html")


@pytest.fixture
def scraper():
    with patch("seas_scraper.PostgresDBManager"):
        s = SeasArticleScraper(headless=True, test_mode=True)
        s.db_manager = MagicMock()
        s.db_manager.filter_new_urls.return_value = []
        yield s


class TestSeasScraper:
    ...
    def test_extract_author_with_by_prefix(self, scraper):
        soup = BeautifulSoup('<div class="news__author-press">By Jane Smith</div>', "html.parser")
    ...
    @patch("seas_scraper.dateparser.parse", return_value=None)
    ...
    # Scrape flow
    @patch("seas_scraper.sync_playwright")
    def test_scrape_flow(self, mock_playwright, scraper, topic_soup, article_soup):
        ...
        scraper.db_manager.filter_new_urls.return_value = ["https://seas.harvard.edu/news/test"]
