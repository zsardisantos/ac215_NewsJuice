...
from hls_scraper import HlsArticleScraper  # noqa: E402
...
def article_soup():
    return _load_html("HLS_sample_article.html")


@pytest.fixture(scope="module")
def topic_soup():
    return _load_html("HLS_sample_topic_page.html")


@pytest.fixture
def scraper():
    with patch("hls_scraper.PostgresDBManager"):
        s = HlsArticleScraper(headless=True, test_mode=True)
        s.db_manager = MagicMock()
        s.db_manager.filter_new_urls.return_value = []
        yield s


class TestHlsScraper:
    ...
    def test_extract_author_with_links(self, scraper):
        soup = BeautifulSoup(
            '<div class="article-byline__author"><a>John Doe</a>, <a>Jane Smith</a></div>',
            "html.parser",
        )
        assert scraper.extract_article_author(soup) == "John Doe, Jane Smith"
    ...
    @patch("hls_scraper.dateparser.parse", return_value=None)
    ...
    # Scrape flow
    @patch("hls_scraper.sync_playwright")
